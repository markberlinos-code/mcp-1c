...
- **Один бинарник, ноль зависимостей.** Написан на Go — не нужен Python, Node.js или JVM. Скачал, запустил, работает.
- **9 инструментов из коробки.** Полный цикл разработки: метаданные, код модулей, формы, запросы, поиск по коду, валидация, журнал регистрации, справка BSL. У аналогов — 3–5.
- **Работает с вашей базой.** AI видит реальную конфигурацию, а не абстрактную справку — метаданные, код модулей, структуру форм, данные через запросы.
- **Структура форм** — уникальная возможность, которой нет у аналогов. AI видит элементы, команды и обработчики событий формы.
...
| `bsl_syntax_help` | Справка по 60+ встроенным функциям и типам языка BSL |
