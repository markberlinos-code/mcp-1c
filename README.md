...
| Tool | Описание |
|------|---------|
| `get_metadata_tree` | Дерево метаданных: справочники, документы, регистры, общие модули |
| `get_object_structure` | Реквизиты, табличные части, измерения и ресурсы конкретного объекта |
| `get_module_code` | Исходный код модуля объекта 1С (модуль объекта, менеджера, формы, общий модуль) |
| `execute_query` | Выполнить запрос на языке запросов 1С (только SELECT/ВЫБРАТЬ) |
| `bsl_syntax_help` | Справка по встроенным функциям BSL |

Поддерживаемые значения `object_type`: `Document`, `Catalog`, `InformationRegister`, `AccumulationRegister`, `AccountingRegister`.
...
MCP-сервер найдёт платформу 1С автоматически, установит расширение и обновит конфигурацию базы данных.
